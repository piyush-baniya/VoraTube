...
class SongActions {
  ...
  static Future<void> _handleAction(
    ...
    switch (action) {
      ...
      case SongAction.suggestMood:
        ...
        break;
      case SongAction.suggestGenre:
        await _suggestGenre(context, ref, tile);
        break;
  ...
  /// Opens the Suggest Genre sheet: shows ALL genre options generated by the
  /// existing genre engine (local cache + iTunes lookup) for this song, lets
  /// the user pick one, and persists it. The genre is a single `songs.genre`
  /// metadata field, so writing the new value replaces the old genre — the
  /// song's previous genre membership is removed by the same write.
  static Future<void> _suggestGenre(
    BuildContext context,
    WidgetRef ref,
    SongTileData tile,
  ) async {
    ...
    final repo = ref.read(libraryRepositoryProvider);
    // A stale sentinel forces suggestGenres() to skip the cache and perform a
    // fresh lookup so the sheet always reflects the engine's current output.
    await GenreEnrichmentService.invalidateCache(
      rowId: song.id,
      writeCache: repo.kvSet,
    );
    Future<List<String>> suggestions() => service.suggestGenres(
      rowId: song.id,
      title: song.title,
      artist: song.artist,
      readCache: repo.kvGet,
    ...
    if (selected == null || selected.isEmpty) return; // cancelled / dismissed
    try {
      await repo.updateSongTags(
        song.id,
        title: song.title,
      ...
      if (context.mounted) _snack(context, 'Genre set to $selected');
      _refreshLibrary(ref);
    } catch (_) {
      if (context.mounted) _snack(context, 'Failed to update genre');
    }
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  _ActionTile(
                    icon: Icons.style_rounded,
                    label: 'Suggest genre',
                    onTap: () => onAction(SongAction.suggestGenre),
                  ),
                  _ActionTile(
                    icon: Icons.mood_rounded,
                    label: 'Suggest mood',
...
/// engine generated for a song and lets the user pick one.
///
/// [suggestionsLoader] fetches the generated options (cache + online lookup
/// via [GenreEnrichmentService]). The list is displayed in full — no
/// hardcoded genres, no cap. When the engine produces nothing, an explicit
/// empty state is shown instead of inventing genres.
///
/// Returns the selected genre via `Navigator.pop(context, genre)`, or `null`
/// when the user cancels or dismisses the sheet.
class GenreSuggestSheet extends StatefulWidget {
  const GenreSuggestSheet({
    super.key,
  ...
  @override
  State<GenreSuggestSheet> createState() => _GenreSuggestSheetState();
}

class _GenreSuggestSheetState extends State<GenreSuggestSheet> {
  String? _selected;
          ...
          children: [
            ...
            Flexible(
              child: FutureBuilder<List<String>>(
                future: widget.suggestionsLoader(),
                builder: (context, snapshot) {
                    ...
                    return Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'No genre suggestions available for this song.',
